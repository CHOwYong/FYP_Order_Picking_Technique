...
class layout():
    ...
    def load(self): #Maybe switch this to load layout one, then have other functions for other layouts, then have a big load function that chooses what to load
        ...
        for i in range(1, self.no_of_rows + 1):
            for j in range(1, self.no_of_columns + 1):
                aisle_arr[i][j] = aisle(self.no_of_sku_per_aisle, j, i)
    ...
    def link_col_aisle(self):
        for row in range(self.no_of_rows):
            ...
            for col in range(self.no_of_columns):
                ...
                self.link_LR(prev_col, curr_col)
                prev_col = curr_col
    ...
    def link_row_aisle(self):
        for col in range(self.no_of_columns):
            ...
            for row in range(self.no_of_rows):
                ...
                self.link_TB(prev_row, curr_row)
                prev_row = curr_row
                
    def link_LR(aisle1:aisle,aisle2:aisle):
        """
    ...
    def link_TB(aisle1:aisle,aisle2:aisle):
        """
