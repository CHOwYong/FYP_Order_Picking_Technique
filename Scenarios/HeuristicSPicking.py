...
def heuristic_s(layout:layout,sku_list):
    ...
    for row in sku_list_rows:
        if ended_at is not None:
            move_down_next_row(dist_pick_row[1],ended_at)
            total_dist += move_down_next_row[0]
            start_aisle = move_down_next_row[1]
            direction = move_down_next_row[2]

        dist_pick_row = heuristic_s_picking_row(row,start_aisle,direction)
        total_dist += dist_pick_row[0]
        ended_at = dist_pick_row[1]

    total_dist += return_to_start((len(sku_list_rows),ended_at))
